...
const ViewCardWithFilter: React.FC<ViewCardProps> = ({
  ...
  const renderDataSource = () => {
    if (daily) {
      setcountValue(dataSource.totalInPreviousDay);
      return;
    } else if (weekly) {
      setcountValue(dataSource.totalInPreviousWeek);
      return;
    } else if (monthly) {
      setcountValue(dataSource.totalInPreviousMonth);
      return;
    } else if (quarterly) {
      setcountValue(dataSource.totalInPreviousQuarter);
      return;
    } else if (annually) {
      setcountValue(dataSource.totalInPreviousYear);
      return;
    } else {
      setcountValue(dataSource.totalInPreviousDay);
      setDaily(true);
